...
describe('PLAYER METHODS', function () {
  ...
  describe('placeShip', function () {
    ...
    it('should throw an error if no direction is specified', () => {
      ...
      expect(handler).to.throw('You left out the direction, please provide it!');
    });
  });
});
...
describe('COMPUTER PLAYER', function () {
  ...
  describe('generateRandomCoordinates', () => {
    const generateRandomCoordinates = require('../game_logic/player_methods').generateRandomCoordinates;
    const generateRandomDirection = require('../game_logic/player_methods').generateRandomDirection;
    let randoCoords;
    beforeEach(() => {
      randoCoords = generateRandomCoordinates();
      randoDir = generateRandomDirection();
    });

    it('should create an array of x y coordinates', () => {
      expect(randoCoords).to.not.be.empty;
    }); 

    it('x y coordinates should be between 0 and 9 inclusive', () => {
      expect(randoCoords[0] >= 0 && randoCoords[0] <= 9).to.be.true; 
      expect(randoCoords[1] >= 0 && randoCoords[1] <= 9).to.be.true; 
    }); 
  });
  
  describe('generateRandomDirection', () => {
    it('should set a vertical or horizontal direction', () => {
      expect(randoDir).to.be.a('string'); 
    }); 
  }); 

  // describe('computerMethods', function () {
  //   const computerFire = require('../game_logic/player_methods').computerFire;
  //   const generateRandomCoordinates = require('../game_logic/player_methods').generateRandomCoordinates;
  //   const generateRandomDirection = require('../game_logic/player_methods').generateRandomDirection;
  ...
  //     const includesCoords = (little, big) => {
  //       const doesIt = little.some(r => big.indexOf(r) >= 0);
  //       console.log('Does It?? ', doesIt);
  //       return doesIt;
  //     }; 
  //     console.log('ship locations: ', ship.locations);
  //     if (includesCoords([9, 7], ship.locations)) {
  //       expect(ship.damage).to.have.length(1); 
  //     } else {
  //       expect(ship.damage).to.be.empty;      }
  //     //      expect(ship.damage).to.have.length(1) || expect(ship.damage).to.be.empty
  //   });

  //   // it('should place a ship at a random coordinate', () => {
